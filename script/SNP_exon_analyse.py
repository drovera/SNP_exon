...
class SNP_exon_analyse:
    ...
    def fit_draw(self):
        ...
        plt.title('minimal distance between SNP and exon en x, normalized cumulative number en y')
        plt.xscale('log')
        ...
        title = 'min distance between SNP inside genes and exons / normalized cumulative number\n'
        title = title + 'Gamma law, shape=' + '{:.5f}'.format(param[0]) + ', scale=' + '{:.0f}'.format(param[1])
        U.gamma_draw_fit(cxb, cyb, U.gamma_cdf, param, title=title)
        bin_nb = 1000
        U.plot_histo(cxb, bin_nb, 'minimal distance SNP - exons', noyticks=True)
        ax = plt.figure().add_subplot()
        stats.probplot(cxb, dist=stats.gamma, sparams=param, plot=ax)
        ax.set_title('minimal distance SNP - exons, gamma with parameters ' + str(param))
        ax.get_lines()[0].set_markersize(3.0)
        print()
        print('Result as parameters of gamma distribution:')
        print("Shape=", param[0])
        print("Scale=", param[1])
